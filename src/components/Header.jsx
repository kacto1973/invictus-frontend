...
import {fetchNotifications, fetchDeleteAllNotifications, fetchReadAllNotifications} from "../services/fetchers.js";
...
const getTimeElapsed = (fecha) => {
    ...
    if (diffInMonths >= 1) {
        ...
        return `Hace ${Math.floor(diffInHours)} ${Math.floor(diffInHours) === 1 ? "hr" : "hrs"}.`;
    } else if (diffInMins < 0) {
        return "Hace 0 mins.";
    } else {
        return `Hace ${Math.floor(diffInMins)} ${Math.floor(diffInMins) === 1 ? "min" : "mins"}.`;
    }
};
...
const Header = ({ label }) => {
    ...
    const counts = {
        "Todo": formatCount(notifications.filter((n) => n.idEstadoNotificacion.nombre !== "Eliminado").length),
        "Leido": formatCount(notifications.filter((n) => n.idEstadoNotificacion.nombre === "Leido").length),
        "Sin leer": formatCount(notifications.filter((n) => n.idEstadoNotificacion.nombre === "Sin leer").length),
    };
    ...
    return (
        <>
            ...
            <div className="w-full h-[5rem] bg-[#E2BDFD] mb-5 absolute top-0 left-0 flex items-center z-50">
                ...
                <div className="mr-[5rem] flex flex-row absolute right-0">
                    ...
                    <div className="bg-white p-2 ml-8 rounded-md relative hover:bg-gray-200 transition duration-200 ease-in-out cursor-pointer"
      ...
      ${showNotifications ? 'opacity-100 translate-y-0' : 'opacity-0 -translate-y-5'}
    `}
                style={{
                    top: "5rem",
                    width: "390px",
                ...
                >
                    {filteredNotifications.map((notif) => (
                        ...
                        >
                            ...
                            >
                                <img
                                    src={`svgs/${notif.idTipoNotificacion.nombre.toLowerCase().replace(/ /g, "_")}.svg`}
                            ...
                            <div style={{ display: "flex", flexDirection: "column", width: "100%" }}>
                                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                                    ...
                                    <div style={{ fontSize: "10px", fontStyle: "italic", fontWeight: 600, color: "#888888" }}>
